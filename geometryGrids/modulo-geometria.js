...
function Esfera(radio){
    this.getPosicion=function(u,v){
        var sin_u = Math.sin(u*Math.PI);
        var sin_v = Math.sin(v*Math.PI);
        var cos_u = Math.cos(u*Math.PI);
        var cos_v = Math.cos(v*Math.PI);

        var x = radio * sin_u * cos_v; 
        var y = radio * cos_u; 
        var z = radio * sin_u * sin_v;
        return [x,y,z];
    }
...
function generarSuperficie(superficie,filas,columnas){
    ...
    for (i=0; i < filas; i++) {
        for (j=0; j < columnas; j++) {
            indexBuffer.push((columnas + 1)*i+j)
            indexBuffer.push((columnas + 1)*i+j+1)
            indexBuffer.push((columnas + 1)*(i+1)+j)
            indexBuffer.push((columnas + 1)*(i+1)+j+1)
        }
    }
